...
class TestArgumentParser:
    ...
    def test_parse_file_args(self) -> None:
        ...
        args = parser.parse_args(
            ["--expr", "ERROR", "--path", "/var/log", "--file-name", "app.log"]
        )
    ...
    def test_parse_output_args(self) -> None:
        ...
        parser = create_argument_parser()
        args = parser.parse_args(
            ["--expr", "ERROR", "--output", "results.log", "--no-path", "--highlight"]
        )
    ...
    def test_parse_display_args(self) -> None:
        ...
        parser = create_argument_parser()
        args = parser.parse_args(["--expr", "ERROR", "--progress", "--stats"])
    ...
    def test_parse_dry_run_args(self) -> None:
        ...
        parser = create_argument_parser()
        args = parser.parse_args(["--expr", "ERROR", "--dry-run"])
        assert args.dry_run is True

        args = parser.parse_args(["--expr", "ERROR", "--dry-run-details"])
    ...
    def test_parse_size_limit_args(self) -> None:
        ...
        parser = create_argument_parser()
        args = parser.parse_args(
            ["--expr", "ERROR", "--max-file-size", "100", "--max-record-size", "64"]
        )
    ...
    def test_parse_processing_args(self) -> None:
        ...
        parser = create_argument_parser()
        args = parser.parse_args(["--expr", "ERROR", "--workers", "8", "--debug"])
...
class TestConfigFileLoading:
    ...
    def test_load_json_config(self, tmp_path: Path) -> None:
        ...
        config_data = {"expr": "ERROR AND Kafka", "ignore_case": True, "path": "/var/log"}
...
class TestConfigurationBuilding:
    ...
    def test_build_full_config(self, tmp_path: Path) -> None:
        ...
        args = parser.parse_args(
            [
                ...
                "--regex",
                "--path",
                str(tmp_path),  # Use tmp_path so validation passes
                "--file-name",
                "app.log",
                "--from",
                "2025-01-01",
    ...
    def test_build_config_cli_overrides_file(self, tmp_path: Path) -> None:
        ...
        config_data = {"expr": "ERROR", "ignore_case": False, "progress": False}
        ...
        parser = create_argument_parser()
        args = parser.parse_args(
            ["--config", str(config_file), "--expr", "WARNING", "--ignore-case", "--progress"]
        )
...
class TestParseArgsFunction:
    ...
    def test_parse_args_with_all_options(self) -> None:
        ...
        config = parse_args(["--expr", "ERROR", "--ignore-case", "--progress", "--stats"])
