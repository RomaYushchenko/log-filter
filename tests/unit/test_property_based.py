...
from src.log_filter.core.exceptions import TokenizationError, ParseError
...
class TestTokenizerProperties:
    ...
    @given(st.lists(st.text(alphabet=string.ascii_lowercase, min_size=1, max_size=10), min_size=1, max_size=5))
...
class TestParserProperties:
    ...
    @given(
        st.lists(st.text(alphabet=string.ascii_letters, min_size=1, max_size=5), min_size=1, max_size=3),
...
class TestEvaluatorProperties:
    ...
    @given(
        ...
        st.text(alphabet=string.ascii_letters, min_size=1, max_size=10).filter(lambda s: s.upper() not in ['AND', 'OR', 'NOT'])
    )
...
class TestConfigurationProperties:
    ...
    @pytest.mark.skip(reason="ProcessingConfig does not have buffer_size parameter yet - feature not implemented")
