...
class ProcessingPipeline:
    ...
    def _process_files(self, files: list, ast: ASTNode, record_filter: RecordFilter) -> None:
        ...
        def _process_files_single_thread() -> None:
            """Process files sequentially in the current process."""
        ...
        if use_multiprocessing:
            ...
            try:
                ...
                logger.warning(
                    "Multiprocessing unavailable in this environment (%s). "
                    "Falling back to single-threaded mode.",
                    exc,
                )
                _process_files_single_thread()
        else:
