...
class TestConfigurationErrors:
    ...
    def test_output_file_in_readonly_directory(self, tmp_path):
        ...
        try:
            # Some environments still allow writes for the current user (e.g. sandbox/ACL behavior).
            try:
                with open(output, "w", encoding="utf-8") as probe:
                    probe.write("probe")
                pytest.skip("Readonly directory permissions are not enforced in this environment")
            except OSError:
                pass
            ...
            try:
                ...
                # Expected in environments where read-only directory is enforced.
                pass
            else:
                # In some environments no output write is attempted (e.g. no matches),
                # or filesystem permissions are relaxed. Ensure no unexpected output.
                try:
                    assert not output.exists()
                except PermissionError:
                    # Access to readonly directory metadata may itself be denied.
                    pass
        finally:
...
class TestInterruptedOperations:
    ...
    def test_keyboard_interrupt_during_processing(self, tmp_path):
        ...
        with patch("log_filter.processing.pipeline._process_file_worker", side_effect=KeyboardInterrupt):
...
class TestEdgeCaseScenarios:
    ...
    def test_symlink_to_missing_file(self, tmp_path):
        ...
        scanner = FileScanner(tmp_path, allowed_extensions={".log"}, file_masks=[])
