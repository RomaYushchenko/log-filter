...
class BufferedLogWriter:
    ...
    def open(self) -> None:
        ...
        """
        try:
            self._file_handle = open(self.output_path, "w", encoding=self.encoding)
        except OSError as e:
            raise FileHandlingError(
                f"Cannot open output file: {self.output_path}", file_path=self.output_path, cause=e
            )
    ...
    def write_record(self, record: LogRecord, source_path: Optional[Path] = None) -> None:
    ...
    def write_result(
        self, result: SearchResult, source_path: Optional[Path] = None, use_highlight: bool = False
    ) -> None:
    ...
    def _flush_unlocked(self) -> None:
        ...
        if not self._file_handle:
            raise FileHandlingError(
                "Cannot flush: output file not open", file_path=self.output_path
            )
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"BufferedLogWriter(output_path={self.output_path}, " f"buffer_size={self.buffer_size})"
        )
