...
def _process_file_worker(args):
    ...
    try:
        ...
        from log_filter.infrastructure.file_handler_factory import FileHandlerFactory
        from log_filter.processing.record_parser import StreamingRecordParser
        from log_filter.domain.filters import CompositeFilter, DateRangeFilter, TimeRangeFilter, AlwaysPassFilter
        from log_filter.statistics.collector import StatisticsCollector
        from log_filter.core.evaluator import evaluate
        ...
        if config.search.time_from or config.search.time_to:
            time_filter = TimeRangeFilter(
                time_from=config.search.time_from,
                time_to=config.search.time_to,
                parser=parser
            )
...
class ProcessingPipeline:
    ...
    def _validate_config(self) -> None:
        ...
        if not self.config.files.search_root.exists():
            raise ConfigurationError(
                f"Root path does not exist: {self.config.files.search_root}"
            )
    ...
    def run(self) -> None:
        ...
        try:
            ...
            for file_meta in file_scanner.scan():
                ...
                if file_meta.should_skip:
                    self.stats.increment_files_skipped(
                        file_meta.skip_reason or "unknown"
                    )
                    logger.debug(
                        f"Skipping {file_meta.path}: {file_meta.skip_reason}"
                    )
                else:
            ...
            self.stats.stop()
            logger.info(
                f"Pipeline completed in {self.stats.stats.duration_seconds:.2f}s"
            )
    ...
    def _process_files(
        ...
        if use_multiprocessing:
            ...
            with ProcessPoolExecutor(max_workers=worker_count) as executor:
                ...
                futures = {
                    executor.submit(_process_file_worker, args): args[0]
                ...
                for future in as_completed(futures):
                    ...
                    try:
                        file_meta_result, matches, stats_dict, matched_records, error = future.result()
                        ...
                        if error:
                            ...
                            if stats_dict:
                                ...
                                self.stats.stats.total_bytes_processed += stats_dict['total_bytes_processed']
                                self.stats.stats.total_lines_processed += stats_dict['total_lines_processed']
                        ...
                        # Format file size
                        file_size_str = f"{file_meta.size_mb:.1f} MB" if file_meta.size_mb < 1024 else f"{file_meta.size_mb/1024:.1f} GB"
            ...
            for args in worker_args:
                ...
                try:
                    file_meta_result, matches, stats_dict, matched_records, error = _process_file_worker(args)
                    file_duration = time.time() - file_start_time
